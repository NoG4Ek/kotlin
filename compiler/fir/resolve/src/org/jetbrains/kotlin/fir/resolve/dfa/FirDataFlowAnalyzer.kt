...
//        return result.takeIf { it.isNotEmpty() }?.let { stability to it }
        ...
        val newResult = mutableListOf<ConeKotlinType>()
        flow.mustAnalysis.lastGraph?.allAliases(variable)?.forEach {
            it.let {
                newResult.addIfNotNull(it.originalType)
                flow.getTypeStatement(variable)?.exactType?.let { st -> newResult += st }
                variable = it.variable
            }
        }
        flow.getTypeStatement(variable)?.exactType?.let { newResult += it }
        return newResult.takeIf { it.isNotEmpty() }?.let { stability to it }
    }
