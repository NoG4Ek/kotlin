...
//			if (markVars)
//				origToNewNode.put(origNode, AliasNode.Companion.cloneAndMarkVarsOnly(origNode));
//			else
//				origToNewNode.put(origNode, AliasNode.Companion.cloneVarsOnly(origNode));
//		}
//
//		for (AliasNode origNode : orig._nodes) {
...
//			for (RealVariableAndType var : otherNode.getVars()) {
...
//		for (AliasNode otherNode : other._nodes) {
//			AliasNode thisNode = fromOtherToThis.get(otherNode);
//			for (RealVariableAndType label : otherNode.getLabelToNode().keySet()) {
//				AliasNode otherTarget = otherNode.getLabelToNode().get(label);
...
//			for (RealVariableAndType var : node.getVars()) {
...
//			}
//			if (otherNode == null ||
//				node.getVars().size() != otherNode.getVars().size() ||
//				node.getLabelToNode().size() != otherNode.getLabelToNode().size() ||
//				node.getLabelFromNode().size() != otherNode.getLabelFromNode().size())
//				return false;
//			fromThisToOther.put(node, otherNode);
//		}
//		for (AliasNode node : _nodes) {
//			AliasNode otherNode = fromThisToOther.get(node);
//			for (RealVariableAndType label : node.getLabelToNode().keySet()) {
//				AliasNode target = node.getLabelToNode().get(label);
//				AliasNode otherTarget = otherNode.getLabelToNode().get(label);
//				if (fromThisToOther.get(target) != otherTarget)
//					return false;
//			}
//			for (RealVariableAndType label : node.getLabelFromNode().keySet()) {
//				AliasNode source = node.getLabelFromNode().get(label);
//				AliasNode otherSource = otherNode.getLabelFromNode().get(label);
...
//				AliasNode newNode = AliasNode.Companion.intersectVars(oldNode1, oldNode2);
